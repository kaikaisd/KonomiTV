...
async def AddReservationAPI(
    ...
    # Mirakurun バックエンド: EDCB を介さず MirakurunReservation を直接作成して返す
    ## MirakurunReservation は録画に必要な番組情報を自前で保持する設計のため、EDCB の EIT[p/f] 相当のフォールバックは行わず、
    ## DB に番組が存在することを必須要件とする
    if edcb is None:
