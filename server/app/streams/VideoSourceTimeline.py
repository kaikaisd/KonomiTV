...
class VideoSourceTimelineResolver:
    ...
    async def findCandidates(cls, anchor: RecordedProgram) -> list[RecordedProgram]:
        ...
        # 話数に加えて同一 EPG 枠であることまで要求し、EPG 表記揺れによる誤った同一話判定を避ける。
        ## 本家では Bangumi の永続 Episode ID が双方にある場合に、より強い同一話の根拠として優先している。
        ## KonomiTV 側は Bangumi 連携を未導入のため、その分岐は取り込んでいない (導入時にここへ追加する)。
        if anchor.episode_number is not None:
            query = query.filter(
