...
class TelegramNotifier:
    ...
    @staticmethod
    async def sendMissedRecordingNotification(
        bot_token: str,
        chat_id: str,
        title: str,
        channel_name: str | None,
        ...
        """
        録画予約がスケジュールされていたが番組終了までに録画が開始されなかった場合に
        「録画ミス」通知を Telegram に送信する。
        サーバーダウン・スキャン漏れ等で Pending のまま end_time を過ぎた予約が対象。
        ...
        escaped_title = TelegramNotifier._escapeMarkdownV2(title)
        escaped_channel = TelegramNotifier._escapeMarkdownV2(channel_name or '不明')
        escaped_date = TelegramNotifier._escapeMarkdownV2(date_jst)
        escaped_start = TelegramNotifier._escapeMarkdownV2(start_time_jst)
        escaped_end = TelegramNotifier._escapeMarkdownV2(end_time_jst)
        escaped_duration = TelegramNotifier._escapeMarkdownV2(str(duration_min))

        text = (
            f'❌ *録画ミス*\n'
            f'📺 *{escaped_title}*\n'
            f'📡 {escaped_channel}  \\|  📅 {escaped_date}  \\|  🕐 {escaped_start}〜{escaped_end} \\({escaped_duration}分\\)'
        )
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                if not response.is_success:
                    logging.error(
                        f'TelegramNotifier: Failed to send missed recording notification. '
                        f'status={response.status_code} body={response.text}'
                    )
                    return False
                logging.info(f'TelegramNotifier: Sent missed recording notification for title="{title}"')
                return True
        except Exception:
            logging.error('TelegramNotifier: Unexpected error while sending missed recording notification.', exc_info=True)
            return False
