            ...
            video: (() => {
                ...
                if (this.playback_mode === 'Live') {
                    ...
                    // 通常のチャンネルの場合
                        ...
                        // BS4K 以外の通常チャンネル (MPEG-2 映像) では、mpeg2toh264 によるオリジナル画質を先頭に追加する
                        // BS4K は HEVC/MMT のため mpeg2toh264 では再生できず、TLV パススルー側で扱う
                        } else {
                            qualities.push({
                ...
                // ビデオ視聴: 録画番組情報がセットされているはず
                    ...
                    const is_mmts_recorded_video = recorded_video.container_format === 'MMT/TLV';
                    ...
                    // MMT/TLV 録画ファイルでは、FFmpeg / tsreadex を通さず元ファイルをそのままプレイヤーへ渡す TLV パススルー画質を追加する
                    if (is_mmts_recorded_video === true) {
                        qualities.push({
                            ...
                            url: `${streaming_api_base_url}/raw-mmts/mpegts`,
                            tlv: {
                                fileSize: recorded_video.file_size,
                            },
                        });
                    // MPEG-2 TS 録画では、mpeg2toh264 によるオリジナル画質を先頭に追加する
                    } else if (is_original_quality_available === true) {
                        qualities.push({
                            name: 'Original (MPEG-2)',
                            type: 'mpeg2toh264',
